    ...
    <div class='footer'>
      &copy; {DateTime.UtcNow.Year} Jannara. All rights reserved.
        ...
        public async Task<Result<bool>> SendAccountConfirmationAsync(string email)
        {
            ...
            ConfirmationTokenDTO accountConfirmationDTO = new ConfirmationTokenDTO(0, userResult.Data.Id, token, code, DateTime.Now.AddMinutes(15), false);
            Result<int> saveTokenResult = await _confirmationTokenService.AddNewAsync(accountConfirmationDTO);
            if (!saveTokenResult.IsSuccess)
            {
                return new Result<bool>(false, "internal_server_error", false, 500);
            }
  ...
  <div class='container'>
    ...
    <div class='footer'>
      &copy; {DateTime.UtcNow.Year} Jannara. All rights reserved.
            ...
            try
            {
                await _emailSenderService.SendEmailAsync(email, "Account Confirmation", body);
